...
import { audioRecorderPlayer, setCurrentlyPlaying } from './App';
...
export async function playRecording(
  name: string,
  playbackListner?: (playbackMeta: PlayBackType) => void,
  onInterrupted?: () => void,
): Promise<boolean> {
...
export function RecordButton({
  ...
  height: number;
  revision: number;
  onSave: (tmpName: string) => void;
}) {
  const [recordProgress, setRecordProgress] = useState(0);
  const [_, setRecordProgressInterval] = useState<NodeJS.Timeout | undefined>(
    undefined,
  );
  ...
  const onStopRecord = async () => {
    const result = await audioRecorderPlayer.stopRecorder().catch(err => {
      setLog(prev => prev + '\nerr stop record: ' + err);
    });
  ...
  return (
    <View style={{ flexDirection: 'column', width: size * 4, height }}>
      <View
        style={{
          flexDirection: 'row',
      ...
      >
        ...
        >
          <AudioWaveForm
            width={size * 2}
            height={40}
            infiniteProgress={recordProgress}
          ...
          >
            {state.recordTime?.substring(0, 5) || '0.0'}
          </Text>
        </View>
      </View>
    </View>
